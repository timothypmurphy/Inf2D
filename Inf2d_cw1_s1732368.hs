...
checkBranchesArrival destination ((x:xs):ys) | checkArrival destination x = (x:xs)
...
depthFirstSearch destination next ((x:xs):ys) exploredList | checkArrival destination x = Just (x:xs)
...
depthLimitedSearch destination next ((x:xs):ys) d | checkArrival destination x = Just (x:xs)
...
iterLoop destination next n ((x:xs):ys) d | checkArrival destination x = Just (x:xs)
...
bestFirstSearch dest next heur ((x:xs):ys) exp | checkArrival dest x = Just (x:xs)
...
aStarSearch dest next heur cost ((x:xs):ys) exp | checkArrival dest x = Just (x:xs)
